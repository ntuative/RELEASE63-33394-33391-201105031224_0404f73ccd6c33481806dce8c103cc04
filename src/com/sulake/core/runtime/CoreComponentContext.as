package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1417:String = "asset-libraries";
      
      private static const const_1419:String = "library";
      
      private static const const_1418:String = "service-libraries";
      
      private static const const_1422:String = "library";
      
      private static const const_1421:String = "component-libraries";
      
      private static const const_1414:String = "library";
      
      private static const const_1415:String = "error_data";
      
      private static const const_1416:String = "error_cat";
      
      private static const const_1420:String = "error_desc";
       
      
      private var var_357:LibraryLoaderQueue;
      
      private var var_769:IEventDispatcher;
      
      private var var_510:uint;
      
      private var var_356:Function;
      
      private var var_207:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1228:
               debug("Core; using simple frame update handler");
               this.var_356 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_875:
               debug("Core; using complex frame update handler");
               this.var_356 = this.complexFrameUpdateHandler;
               break;
            case Core.const_795:
               debug("Core; using profiler frame update handler");
               this.var_356 = this.profilerFrameUpdateHandler;
               this.var_207 = new Profiler(this);
               attachComponent(this.var_207,[new IIDProfiler()]);
               var_402.addChild(new ProfilerViewer(this.var_207));
               break;
            case Core.const_555:
               debug("Core; using experimental frame update handler");
               this.var_356 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_356 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_357.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_510 - this.getNumberOfFilesPending();
      }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1415,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1416,String(param3));
         ErrorReportStorage.setParameter(const_1420,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         if(this.var_357 == null)
         {
            this.var_357 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1417)[0];
            if(node != null)
            {
               list = node.child(const_1419);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_351,e);
         }
         try
         {
            node = config.child(const_1418)[0];
            if(node != null)
            {
               list = node.child(const_1422);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_351,e);
         }
         try
         {
            node = config.child(const_1421)[0];
            if(node != null)
            {
               list = node.child(const_1414);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_351,e);
         }
         if(!disposed)
         {
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1606);
         if(!disposed)
         {
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_769)
                  {
                     this.var_769.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_510 - this.var_357.length,this.var_510,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_207 ? 0 : this.var_1459 & 0;
         if(_loc3_ == Core.const_555)
         {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_356(_loc2_,_loc2_ - this._lastUpdateTimeMs);
         ...
         while(priority < CoreComponentContext.const_109)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_207.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_207)
            {
               detachComponent(this.var_207);
               this.var_207.dispose();
               this.var_207 = null;
            }
            switch(this.var_1459 & 0)
            {
               case Core.const_1228:
                  this.var_356 = this.simpleFrameUpdateHandler;
                  break;
