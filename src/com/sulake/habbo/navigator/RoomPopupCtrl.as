package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2660:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2660 = new RoomThumbnailRenderer(param1);
         this.var_1116 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_184 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_184.visible = true;
         Util.hideChildren(this.var_184);
         this.refreshRoomName(this.var_184,this.var_117);
         if(this.var_117.ownerName != "" && this.var_117.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_184,!!this.var_117.event ? this.getEventCtg(this.var_117.categoryId) : this.getRoomCtg(this.var_117.categoryId));
         this.refreshRoomDesc(this.var_184,this.var_117);
         this.refreshExtraCont();
         this.refreshInfo(this.var_184,"trading_allowed",this.var_117.allowTrading);
         this.refreshInfo(this.var_184,"doormode_doorbell",this.var_117.doorMode == 1);
         this.refreshInfo(this.var_184,"doormode_password",this.var_117.doorMode == 2);
         Util.moveChildrenToColumn(this.var_184,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_184.height = Util.getLowestPoint(this.var_184);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_184,this.var_117.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_184.findChildByName("roomowner"));
         if(this.var_117.ownerName != "" && this.var_117.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_184.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1116.refreshTags(_loc1_,this.var_117.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_117.score);
         if(this.var_117.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_117.eventCreationTime);
         }
         this.var_2660.refreshThumbnail(_loc1_,this.var_117.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_282,Tabs.const_276,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
