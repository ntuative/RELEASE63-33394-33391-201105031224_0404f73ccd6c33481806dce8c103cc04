package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1194:IDropMenuWindow;
      
      private var var_511:ITextFieldWindow;
      
      private var var_584:Boolean = true;
      
      private var var_981:ICheckBoxWindow;
      
      private var var_1504:ICheckBoxWindow;
      
      private var var_1505:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_33.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_374));
         Logger.log("BEGINNING TO SHOW: " + this.var_374);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1568;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_374);
         if(param1.flatId != this.var_374)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_374);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      public function populate() : void
      {
         ...
         this.var_511 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_511.procedure = this.onInputClick;
         this.var_1194 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1194);
         this.var_1194.procedure = this.onSelectTemplate;
         this.var_981 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1504 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1505 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_33.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_33,WindowTracker.const_824,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_584 || this.var_511.text == "")
         {
            this.var_33.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_981.isSelected);
         this.var_33.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_1699,_loc2_,this.var_511.text,"",""));
         if(this.var_1504.isSelected || this.var_1505.isSelected || this.var_981.isSelected)
         {
            this.var_33.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1504.isSelected,this.var_1505.isSelected,this.var_981.isSelected));
         }
         this.dispose();
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_33.initMsg.roomMessageTemplates[this.var_1194.selection];
         if(_loc3_ != null)
         {
            this.var_584 = false;
            this.var_511.text = _loc3_;
         }
      }
   }
}
