package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_1534:IWindow;
      
      private var var_820:IFigurePartSet;
      ...
      private var var_663:Rectangle;
      
      private var var_1954:int = 0;
      
      private var var_444:IAvatarRenderManager;
      
      private var var_1080:Boolean;
      
      private var var_1956:BitmapData;
      
      private const const_2133:int = 13421772;
      
      private const const_2134:int = 16777215;
      
      private const const_368:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_820 = param3;
         this._window = param1;
         this.var_1534 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1955 = param5;
         if(param3 == null)
         {
            this.var_708 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1954 = Math.max(this.var_1954,_loc6_.colorLayerIndex);
            }
         }
         this.var_444 = this.var_67.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_708 != null && !this.var_1955)
            {
               _loc3_ = this.var_708;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_820)
            {
               switch(this.var_820.clubLevel)
               {
                  case HabboClubLevelEnum.const_56:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_839;
                     break;
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_970;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1534.color = this.const_2133;
         }
         else
         {
            this.var_1534.color = this.const_2134;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_444.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_444.isFigureReady(_loc1_))
         {
            this.var_444.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_368[_loc6_] + "_" + FigureData.const_156;
               _loc4_ = this.var_444.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_368.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_368[_loc6_] + "_" + FigureData.const_156;
                  _loc4_ = this.var_444.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_663 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1956)
               {
                  ...
                  this.var_1956 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_663.width,this.var_663.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_663.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_663.y;
