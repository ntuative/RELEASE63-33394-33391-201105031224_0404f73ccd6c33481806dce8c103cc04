package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_423:AssetLoaderStruct;
      ...
      private const const_2136:int = 100;
      
      private const const_1792:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_423 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_932 = new Timer(this.const_2136,1);
         this.var_932.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_423)
         {
            this.var_423.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_423.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_423)
         {
            this.var_423.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_423.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1444.indexOf("?") > 0)
            {
               _loc2_ = this.var_1444 + "&retry=" + this.var_931;
            }
            else
            {
               _loc2_ = this.var_1444 + "?retry=" + this.var_931;
            ...
            this.var_423 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1780)
         {
            this.var_1162.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_492[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_492[_loc3_] = _loc6_;
            }
            this.var_933[_loc3_] = _loc4_;
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_933)
         {
            _loc7_ = true;
            _loc4_ = this.var_933[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_492[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_933[_loc3_];
         }
         while(_loc6_ < this.var_755.length)
         {
            _loc5_ = this.var_755[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_755.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_932.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_756.length > 0 && this.var_755.length < this.const_1792)
         {
            _loc1_ = this.var_756[0];
            _loc1_.startDownloading();
            this.var_755.push(this.var_756.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_756.indexOf(param1) == -1 && this.var_755.indexOf(param1) == -1)
         {
            this.var_756.push(param1);
            this.processPending();
         }
      }
